...
class CommitPlan:
    ...
    submodule_targets: Dict[str, str] = field(default_factory=dict)  # submodule_path -> target_commit
...
class RebasePlan:
    ...
    submodule_mappings: Dict[str, Dict[str, str]] = field(default_factory=dict)  # submodule_path -> {old_commit -> new_commit}
...
class RebasePlanner:
    ...
    def create_rebase_plan(
        ...
        plan.submodule_mappings = self._build_submodule_mappings(repo_info, submodule_results)
        ...
        for i, commit in enumerate(commits_to_rebase):
            commit_plan = self._create_commit_plan(repo_info, commit, plan.submodule_mappings)
            plan.commit_plans.append(commit_plan)
            
            if self.git_utils.verbose:
                submodule_count = len(commit_plan.submodule_targets)
                print(f"   📝 Commit {i+1}/{len(commits_to_rebase)}: {commit[:8]} ({submodule_count} submodules)")
    ...
    def _build_submodule_mappings(
        ...
        for result in submodule_results:
            ...
            try:
                ...
                if self.git_utils.verbose:
                    print(f"   🔗 Mapped submodule: {rel_path} ({len(result.commit_mapping)} commits)")
            except ValueError:
                # Paths are on different drives or not related
                if self.git_utils.verbose:
                    print(f"   ⚠️  Skipping unrelated submodule: {result.repo_path}")
                continue
    ...
    def _get_commits_to_rebase(self, repo_info: RepoInfo) -> List[str]:
        ...
        """
        try:
            # Get commits between base and branch (oldest first for rebase order)
    ...
    def _create_commit_plan(
        ...
        submodules_in_commit = self._get_submodules_in_commit(repo_info, commit)
        ...
        for submodule_path in submodules_in_commit:
            ...
            if target_commit:
                ...
                if self.git_utils.verbose:
                    old_commit = self._get_submodule_commit_in_commit(repo_info, commit, submodule_path)
                    print(f"     📦 {submodule_path}: {old_commit[:8] if old_commit else 'None'} -> {target_commit[:8]}")
        
        return plan
    
    def _get_submodules_in_commit(self, repo_info: RepoInfo, commit: str) -> List[str]:
    ...
    def _calculate_submodule_target(
        ...
        for path_variant in [submodule_path, normalized_path, submodule_path.replace('/', '\\')]:
            if path_variant in submodule_mappings:
                ...
                # Try to find mapping for this commit (both short and full hash)
                ...
                if os.path.exists(submodule_full_path):
                    try:
                        # Check if the old commit still exists (might be unchanged)
                        self.git_utils.sh([
        ...
        if self.git_utils.verbose:
            print(f"     ⚠️  No mapping found for {submodule_path}:{old_submodule_commit[:8]}")
        
        return None
    
    def get_plan_for_commit(self, plan: RebasePlan, old_commit: str) -> Optional[CommitPlan]:
        ...
        for commit_plan in plan.commit_plans:
            if commit_plan.old_commit.startswith(old_commit) or old_commit.startswith(commit_plan.old_commit):
                return commit_plan
        return None
