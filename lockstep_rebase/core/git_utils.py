...
class GitUtils:
    ...
    def run(self, cmd: List[str], cwd: Optional[str] = None, capture: bool = False, env: Optional[Dict] = None) -> str:
        ...
        if cmd and cmd[0] == 'git':
            final_env['GIT_EDITOR'] = 'true'  # 'true' command always succeeds and does nothing
        
        if capture:
            result = subprocess.check_output(cmd, cwd=cwd, env=final_env, stderr=subprocess.STDOUT)
            return result.decode().strip()
        else:
            subprocess.check_call(cmd, cwd=cwd, env=final_env)
            return ""
    
    def sh(self, cmd: List[str], cwd: Optional[str] = None, capture: bool = False) -> str:
    ...
    def find_git_root(self, start_path: str) -> Optional[str]:
        ...
        """
        try:
            output = self.sh(['git', 'rev-parse', '--show-toplevel'], cwd=start_path, capture=True)
            if self.verbose:
    ...
    def get_current_branch(self, repo_path: str) -> str:
        ...
        """
        try:
            return self.sh(['git', 'symbolic-ref', '--short', 'HEAD'], cwd=repo_path, capture=True)
        except subprocess.CalledProcessError:
            # Might be in detached HEAD state
            return self.sh(['git', 'rev-parse', '--short', 'HEAD'], cwd=repo_path, capture=True)
    ...
    def get_branches(self, repo_path: str) -> List[str]:
        ...
        """
        try:
            output = self.sh(['git', 'branch', '-a'], cwd=repo_path, capture=True)
            branches = []
    ...
    def get_conflicted_files(self, repo_path: str) -> tuple[List[str], List[str]]:
        ...
        """
        try:
            status_output = self.sh(['git', 'status', '--porcelain=v1'], cwd=repo_path, capture=True)
            regular_conflicts = []
            submodule_conflicts = []
            ...
            for line in status_output.split('\n'):
                if line.startswith('UU '):
                    ...
                    try:
                        self.sh(['git', 'ls-files', '--stage', file_path], cwd=repo_path, capture=True)
                        # If we can get stage info, check if it's a submodule (mode 160000)
                        stage_info = self.sh(['git', 'ls-files', '--stage', file_path], cwd=repo_path, capture=True)
    ...
    def find_backup_branches(self, repo_path: str) -> List[str]:
        ...
        """
        try:
            branches_output = self.sh(['git', 'branch'], cwd=repo_path, capture=True)
            backup_branches = []
