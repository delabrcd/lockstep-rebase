...
class NestedRebaseManager:
    ...
    def __init__(self, dry_run: bool = False, verbose: bool = False):
        ...
        # repo_path -> rebase_plan
        self.rebase_plans: Dict[str, RebasePlan] = {}
    ...
    def run_nested_rebase(self, root_path: str) -> None:
        ...
        if existing_state:
            ...
            print(
                f"   Completed rebases: {len(existing_state.completed_rebases)}")
    ...
    def _configure_single_repository(self, repo_info: RepoInfo) -> None:
        ...
        try:
            ...
            if InteractivePrompt.yes_no(
                f"Do you want to rebase {repo_info.name}?",
                    default=False):
                ...
                if selected_branch:
                    ...
                    # Get base branch/commit with bash-style inline
                    # autocomplete
                    ...
                    if base_branch:
                        ...
                        try:
                            self.git_utils.sh(
                                ['git', 'rev-parse', base_branch], cwd=repo_info.path, capture=True)
                            repo_info.base = base_branch
                            print(f"✅ Selected base: {base_branch}")
                        except subprocess.CalledProcessError:
                            # Base might be a commit hash or tag, try manual
                            # input
                            print(
                                f"⚠️  '{base_branch}' not found as a branch, trying as commit/tag...")
                            base_commit = self._get_base_commit_manual(
                                repo_info, selected_branch)
                            if base_commit:
                                repo_info.base = base_commit
                            else:
                                print(
                                    f"❌ Could not configure base for {repo_info.name}")
                                return
                    else:
    ...
    def _get_base_commit_manual(
        ...
        while True:
            try:
                ...
                # Validate that the base exists
                try:
                    self.git_utils.sh(['git', 'rev-parse', base],
                                      cwd=repo_info.path, capture=True)
                    print(f"✅ Valid base: {base}")
                    return base
                except subprocess.CalledProcessError:
                    print(
                        f"❌ Base '{base}' not found. Please enter a valid branch/commit/tag.")
    ...
    def _rebase_tree_recursive(
            self, repo_info: RepoInfo) -> List[RebaseResult]:
        ...
        # Then rebase this repository, passing all previous results for
        # conflict resolution
    ...
    def _rebase_repository(
            self,
            repo_info: RepoInfo,
            all_results: List[RebaseResult]) -> RebaseResult:
        ...
        try:
            ...
            rebase_plan = self.planner.create_rebase_plan(
                repo_info, all_results)
            self.rebase_plans[repo_info.path] = rebase_plan

            # Checkout the target branch
            if not self.dry_run:
                self.git_utils.sh(
                    ['git', 'checkout', repo_info.branch], cwd=repo_info.path)
            ...
            if not self.dry_run:
                ...
                self.git_utils.sh(
                    ['git', 'branch', backup_branch], cwd=repo_info.path)
                print(f"📦 Created backup branch: {backup_branch}")

            # Perform the rebase
            if self.dry_run:
                print(
                    f"[DRY RUN] Would rebase {repo_info.branch} onto {repo_info.base}")
                if rebase_plan.commit_plans:
                    print(
                        f"[DRY RUN] Plan includes {len(rebase_plan.commit_plans)} commits with submodule mappings")
                result = RebaseResult(
                    repo_path=repo_info.path,
                    success=True,
                    commit_mapping={},
                ...
                success, commit_mapping = self._execute_planned_rebase(
                    repo_info, rebase_plan)
    ...
    def _execute_planned_rebase(
            self, repo_info: RepoInfo, rebase_plan: RebasePlan) -> tuple[bool, Dict[str, str]]:
        ...
        if self.verbose:
            print(
                f"🎯 Executing planned rebase with {len(rebase_plan.commit_plans)} commits")
        ...
        try:
            # Start the rebase
            self.git_utils.sh(
                ['git', 'rebase', repo_info.base], cwd=repo_info.path)
            print(f"✅ Rebase completed successfully without conflicts!")

            # Get the final commit mapping
            commit_mapping = self._extract_commit_mapping_after_rebase(
                repo_info, rebase_plan)
    ...
    def _resolve_rebase_with_plan(
        ...
        while attempt < max_attempts:
            ...
            if not self.git_utils.is_rebase_in_progress(repo_info.path):
                if self.verbose:
                    print(
                        f"✅ Rebase completed successfully after {attempt-1} conflict resolutions!")
                commit_mapping = self._extract_commit_mapping_after_rebase(
                    repo_info, rebase_plan)
                return True, commit_mapping
            ...
            if self.planned_resolver.resolve_conflicts_with_plan(
                    repo_info, rebase_plan):
                ...
                try:
                    ...
                    if self.verbose:
                        print(
                            f"⚠️  Rebase --continue failed, checking for remaining conflicts")
                    ...
                    if regular_conflicts:
                        # We have non-submodule conflicts that need manual
                        # resolution
                        print(
                            f"❌ Found {len(regular_conflicts)} non-submodule conflicts requiring manual resolution")
                        if not self.interactive_handler.handle_merge_conflicts(
                                repo_info):
                            return False, {}
                        continue
                    elif submodule_conflicts:
                        # Still have submodule conflicts - our plan might be
                        # incomplete
                        if self.verbose:
                            print(
                                f"⚠️  Still have {len(submodule_conflicts)} unresolved submodule conflicts")
                        continue  # Try again with the planned resolver
                    else:
                        # No conflicts but rebase --continue failed - might be
                        # done
                        if self.verbose:
                            print(
                                f"⚠️  No conflicts detected but rebase --continue failed")
                        break
            else:
        ...
        print(
            f"❌ Rebase failed: exceeded maximum conflict resolution attempts ({max_attempts})")
        return False, {}

    def _extract_commit_mapping_after_rebase(
            self, repo_info: RepoInfo, rebase_plan: RebasePlan) -> Dict[str, str]:
        ...
        try:
            ...
            new_commits = [
                line.strip() for line in new_commits_output.split('\n') if line.strip()]
        ...
        while True:
            ...
            try:
                ...
                if submodule_conflicts:
                    print(
                        f"🤖 Found {len(submodule_conflicts)} submodule conflicts, attempting auto-resolution...")
                    ...
                    print(
                        f"❌ Found {len(regular_conflicts)} non-submodule conflicts - manual resolution required")
                    return False
    ...
    def _summarize_changes(self, results: List[RebaseResult]) -> None:
        ...
        successful_rebases = [
            r for r in results if r.success and r.commit_mapping]
        skipped_rebases = [
            r for r in results if r.success and not r.commit_mapping]
        ...
        if skipped_rebases:
            ...
            for result in skipped_rebases:
                repo_name = os.path.basename(result.repo_path)
                print(
                    f"  📁 {repo_name}: {result.error_message or 'No changes needed'}")
    ...
    def _push_all_changes(
        ...
        for result in successful_results:
            ...
            if not branch:
                print(
                    f"⚠️  Could not find branch for {repo_name}, skipping push")
                continue
    ...
    def abort_all_active_rebases(self) -> None:
        ...
        for repo_path in self.active_rebases[:]:
            try:
                # Check if there's actually a rebase in progress
                status_output = self.git_utils.sh(
                ...
                if self.git_utils.is_rebase_in_progress(repo_path):
                    ...
                    self.git_utils.sh(
                        ['git', 'rebase', '--abort'], cwd=repo_path)
