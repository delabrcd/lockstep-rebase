...
class BackupManager:
    ...
    def _delete_all_backups(self, repo_backups: Dict[str, List[str]]) -> None:
        ...
        for repo_path, backups in repo_backups.items():
            ...
            for backup in backups:
                try:
                    self.git_utils.sh(['git', 'branch', '-D', backup], cwd=repo_path)
    ...
    def _delete_backups_by_repo(self, repo_backups: Dict[str, List[str]]) -> None:
        ...
        while True:
            ...
            if selection.lower() == 'all':
                ...
                try:
                    ...
                    selected_repos = [repo_list[i] for i in indices if 0 <= i < len(repo_list)]
                    if selected_repos:
                        break
                    else:
    ...
    def _delete_specific_backups(self, repo_backups: Dict[str, List[str]]) -> None:
        ...
        while True:
            ...
            try:
                indices = [int(x.strip()) - 1 for x in selection.split(',')]
                selected_backups = [all_backups[i] for i in indices if 0 <= i < len(all_backups)]
                if selected_backups:
                    break
                else:
    ...
    def prompt_cleanup_backups_after_push(self, results: List[RebaseResult]) -> None:
        ...
        for result in results:
            if result.success and result.backup_branch:
                ...
                backup_branches.append((result.repo_path, repo_name, result.backup_branch))
        ...
        while True:
            ...
            if cleanup in ['y', 'yes']:
                ...
                for repo_path, repo_name, backup_branch in backup_branches:
                    try:
                        self.git_utils.sh(['git', 'branch', '-D', backup_branch], cwd=repo_path)
                        print(f"  ✅ Deleted {repo_name}: {backup_branch}")
                    except subprocess.CalledProcessError as e:
                        print(f"  ❌ Failed to delete {repo_name}: {backup_branch} - {e}")
                break
