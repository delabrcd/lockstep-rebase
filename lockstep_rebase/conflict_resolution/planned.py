...
class PlannedSubmoduleResolver:
    ...
    def resolve_conflicts_with_plan(
        ...
        commit_plan = self._find_current_commit_plan(repo_info, plan, current_commit)
        ...
        if self.git_utils.verbose:
            print(f"   📝 Using plan for commit: {commit_plan.old_commit[:8]} - {commit_plan.old_commit_message}")
        
        # Resolve each conflicted submodule
        resolved_count = 0
        for submodule_path in conflicted_files:
            if self._resolve_submodule_with_plan(repo_info, submodule_path, commit_plan):
                resolved_count += 1
        ...
        if self.git_utils.verbose:
            if success:
                print(f"   ✅ Successfully resolved all {resolved_count} submodule conflicts")
            else:
                print(f"   ⚠️  Resolved {resolved_count}/{len(conflicted_files)} submodule conflicts")
    ...
    def _find_current_commit_plan(
        ...
        try:
            ...
            rebase_head_name = os.path.join(repo_info.path, '.git', 'rebase-merge', 'head-name')
            rebase_onto = os.path.join(repo_info.path, '.git', 'rebase-merge', 'onto')
            
            if os.path.exists(rebase_head_name) and os.path.exists(rebase_onto):
                ...
                try:
                    ...
                    # Find matching plan
                    for commit_plan in plan.commit_plans:
                        if (commit_plan.old_commit.startswith(rebase_head) or 
                            rebase_head.startswith(commit_plan.old_commit)):
                            return commit_plan
                except subprocess.CalledProcessError:
                    pass
        except Exception:
    ...
    def _resolve_submodule_with_plan(
        ...
        if not target_commit:
            if self.git_utils.verbose:
                print(f"     ❌ No target found for {submodule_path} in commit plan")
            return False
    ...
    def _fallback_resolution(
        ...
        for submodule_path in conflicted_files:
            ...
            if self._fallback_resolve_submodule(repo_info, submodule_path, plan):
                resolved_count += 1
        ...
        if self.git_utils.verbose:
            if success:
                ...
                print(f"   ⚠️  Fallback resolved {resolved_count}/{len(conflicted_files)} conflicts")
    ...
    def _apply_submodule_resolution(
        ...
        try:
            ...
            if self.git_utils.verbose:
                print(f"     ✅ Applied resolution: {submodule_path} -> {target_commit[:8]}")
