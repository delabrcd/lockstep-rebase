...
class SubmoduleConflictResolver:
    ...
    def resolve_conflicts_automatically(
        ...
        for submodule_path in submodule_conflicts:
            if self._resolve_single_submodule_conflict(submodule_path, repo_info, submodule_mappings):
                resolved_count += 1
    ...
    def _build_submodule_commit_mappings(
        ...
        if self.git_utils.verbose:
            ...
            for path, mapping in saved_mappings.items():
                rel_path = os.path.relpath(path, repo_info.path) if os.path.isabs(path) else path
                print(f"     {rel_path}: {len(mapping)} commits")
            print(f"   Current results: {len(all_results)} repositories")
            for result in all_results:
                rel_path = os.path.relpath(result.repo_path, repo_info.path) if os.path.isabs(result.repo_path) else result.repo_path
                print(f"     {rel_path}: {len(result.commit_mapping)} commits (success: {result.success})")
        ...
        for submodule in all_submodules:
            ...
            if abs_path in saved_mappings:
                ...
                if self.git_utils.verbose:
                    print(f"     📂 Using saved mapping: {len(saved_mappings[abs_path])} commits")
                continue
            
            # Otherwise, find the rebase result for this submodule from current session
            found_mapping = False
            for result in all_results:
                ...
                if result_abs_path == abs_path:
                    if result.success and result.commit_mapping:
                        ...
                        found_mapping = True
                        if self.git_utils.verbose:
                            print(f"     🔄 Using current session mapping: {len(result.commit_mapping)} commits")
                    else:
                        if self.git_utils.verbose:
                            print(f"     ⚠️  Found result but no mapping (success: {result.success}, mapping: {len(result.commit_mapping)})")
                    break
    ...
    def _print_available_mappings(self, submodule_mappings: Dict[str, Dict[str, str]]) -> None:
    ...
    def _resolve_single_submodule_conflict(
        ...
        try:
            ...
            try:
                rebase_head = self.git_utils.sh(['git', 'rev-parse', 'REBASE_HEAD'], cwd=repo_info.path, capture=True)
                ...
                if self.git_utils.verbose:
                    print(f"   Rebase target submodule commit: {target_commit}")
                    try:
            ...
            self.git_utils.sh(['git', 'add', submodule_path], cwd=repo_info.path)
            if self.git_utils.verbose:
    ...
    def _try_resolve_with_mapping(
        ...
        if self.git_utils.verbose:
            print(f"   🔍 Looking for mapping: '{submodule_path}' (normalized: '{normalized_path}')")
            print(f"   Available mapping keys: {list(submodule_mappings.keys())}")
        
        # Try different path formats
        mapping = None
        for path_variant in [submodule_path, normalized_path, submodule_path.replace('\\', '/')]:
        ...
        if stage3_commit:
            ...
            if self.git_utils.verbose:
                print(f"   Stage 3 (incoming) commit: {stage3_short} (full: {stage3_full})")
            ...
            for old_commit, new_commit in mapping.items():
                if (old_commit == stage3_short or 
                    ...
                    if self.git_utils.verbose:
                        print(f"   ✅ Found mapping (short hash): {old_commit} -> {new_commit}")
                    break
            
            if mapped_commit:
                return self._apply_mapping(submodule_path, repo_info, mapped_commit)
            else:
                if self.git_utils.verbose:
                    print(f"   ⚠️  No mapping found for Stage 3 commit {stage3_short}")
                    print(f"   Available mappings: {[f'{k}->{v}' for k, v in mapping.items()]}")
        
        return False
    
    def _apply_mapping(self, submodule_path: str, repo_info: RepoInfo, mapped_commit: str) -> bool:
        ...
        try:
            ...
            self.git_utils.sh(['git', 'checkout', mapped_commit], cwd=submodule_full_path)
            
            # Add the submodule to resolve the conflict
            self.git_utils.sh(['git', 'add', submodule_path], cwd=repo_info.path)
            
            if self.git_utils.verbose:
                print(f"   ✅ Updated {submodule_path} to rebased commit {mapped_commit}")
