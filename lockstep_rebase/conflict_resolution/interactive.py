...
class InteractiveConflictHandler:
    ...
    def handle_merge_conflicts(self, repo_info: RepoInfo) -> bool:
        ...
        if submodule_conflicts:
            return self.handle_submodule_conflicts(repo_info, submodule_conflicts)
        ...
        while True:
            ...
            if choice == '1':
                ...
                try:
                    self.git_utils.sh(['git', 'rebase', '--abort'], cwd=repo_info.path)
                ...
                print("   Resolve conflicts manually, then return here and choose option 1")
                input("\nPress Enter when you're ready to continue...")
            else:
                print("Please enter 1, 2, or 3")
    
    def handle_submodule_conflicts(self, repo_info: RepoInfo, submodule_conflicts: List[str]) -> bool:
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    try:
                        ...
                        self.git_utils.sh(['git', 'checkout', stage3_commit], cwd=submodule_full_path)
                        self.git_utils.sh(['git', 'add', submodule_path], cwd=repo_info.path)
    ...
    def _wait_for_conflict_resolution(self, repo_info: RepoInfo) -> bool:
        ...
        while True:
            ...
            if regular_conflicts or submodule_conflicts:
                ...
                if regular_conflicts:
                    print(f"   📄 File conflicts: {', '.join(regular_conflicts)}")
                if submodule_conflicts:
                    print(f"   📦 Submodule conflicts: {', '.join(submodule_conflicts)}")
