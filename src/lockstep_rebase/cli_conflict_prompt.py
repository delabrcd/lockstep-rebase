...
class CliConflictPrompt(ConflictPrompt):
    ...
    def prompt_for_conflict_resolution(
        ...
        if unresolved_submodule_conflicts:
            self.console.print(
                f"\n📦 **Submodule Conflicts** ({len(unresolved_submodule_conflicts)}):",
                style="bold yellow",
            )
        ...
        instructions_panel = Panel(
            "\n".join(instructions), title="Instructions", title_align="left", border_style="blue"
        )
        ...
        if user_input == "resolved":
            # Core will verify; UI simply signals intent
            return True
        else:
    ...
    def display_resolution_summary(self, summary: ResolutionSummary) -> None:
        ...
        if summary.message_consistency_issues:
            ...
            total_resolutions = sum(
                len(commits) for commits in summary.resolved_commits_by_repo.values()
            )
            self.console.print(
                f"\n✅ **All {total_resolutions} resolved commits have consistent messages.**",
                style="bold green",
            )

    def show_messages(self, messages: List[str], style: str = "") -> None:
        """Display generic messages from core logic."""
        rich_style = style or None
        for m in messages:
            self.console.print(m, style=rich_style)
