...
class CommitTracker:
    ...
    def map_commits(
        self, original_commits: List[CommitInfo], new_commits: List[CommitInfo]
    ) -> Dict[str, str]:
        """
        ...
        for i, original in enumerate(original_commits):
            best_match = self._find_best_match(
                original, new_commits[i : i + 3]
            )  # Look at next 3 commits
            ...
            if best_match:
                ...
                logger.warning(
                    f"Could not find mapping for commit {original.hash[:8]}: {original.message[:50]}"
                )
    ...
    def _find_best_match(
        self, original_commit: CommitInfo, candidate_commits: List[CommitInfo]
    ) -> Optional[CommitInfo]:
        ...
        for candidate in candidate_commits:
            if (
                candidate.message == original_commit.message
                and candidate.author == original_commit.author
            ):
                return candidate
        ...
        for candidate in candidate_commits:
            if (
                self._messages_similar(original_commit.message, candidate.message)
                and candidate.author == original_commit.author
            ):
                return candidate
